 ...
 *
 * 判定：
 *   supported   綠：有核定事實直接支持，數字、否定與條件都一致
 *   partial     黃：部分支持或需確認（語氣被放大、條件遺失、相似度不足）
 *   unsupported 紅：找不到支持的事實，或與事實矛盾 → 不得進入最終發布稿
 */
...
/**
 * 以字元 bigram 的覆蓋率衡量「這句話有多少內容來自該事實」。
 * 分母用句子本身，因此事實比句子長不會被稀釋。
 */
...
export function verifySentence(
  ...
  if (missingNumbers.length > 0) {
    reasons.push(`數字或單位在核定事實中找不到：${missingNumbers.join("、")}`);
  }
  ...
  if (factHedged && !sentenceHedged) {
    reasons.push("核定事實帶有不確定語氣，回答卻寫成確定語氣");
  }

  // 事實帶條件但句子沒有帶出「同一個」條件。
  // 只檢查「有沒有條件詞」會誤放：事實寫「孕婦」、句子寫「每週」也會被當成有保留。
  ...
  if (missingConditions.length > 0) {
    reasons.push(`核定事實的適用條件未保留：${missingConditions.join("、")}`);
  }
