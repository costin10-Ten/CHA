...
以「經人工核定的原子命題」為基礎的個人知識庫。AI 只負責拆解與草擬，核定權在使用者手上；
所有對外內容都必須能回溯到核定原子命題與其原始來源片段。
...
| 3     | Edge Function 抽取候選原子命題、Mock Provider、JSON Schema、品質檢查 | ✅ 完成 |
| 4     | 單人審核介面（核定、修正、駁回、拆分、合併）、review_records         | ✅ 完成 |
| 5     | 正式原子命題庫、實體與關聯、版本管理、pgvector 增量索引              | ✅ 完成 |
...
app/                Next.js App Router 頁面與 Route Handlers
  ...
  review/           候選原子命題審核與抽取問題回報
  knowledge/        正式原子命題與版本
  search/ ask/      混合搜尋與 AI 問答
  verify/           逐句驗證
  generate/         風險溝通素材產製與草稿
  export/           匯出頁與待選原子命題包回填
  import/           文章包匯入與示範資料
...
lib/
  ...
  facts/            候選原子命題查詢、審核規則與標籤
  knowledge/        正式原子命題、實體、關聯查詢
  retrieval/        混合搜尋
  answering/        問答紀錄查詢
  generate/         素材查詢與標籤
...
未設定時使用 Mock Provider：不呼叫任何外部 API，把段落逐句轉成候選原子命題，
可完整跑過抽取、品質檢查與審核流程。單元測試與 CI 一律走這條路徑。

## 單人審核

`/review` 是候選原子命題清單（可篩選、可勾選批次操作），`/review/[id]` 是單筆審核畫面。
...
| 駁回         | 狀態改為已駁回                                                                                 |
| 標記待確認   | 狀態改為待修正                                                                                 |
| 拆成多筆     | 一行一筆建立多筆待審核原子命題，原原子命題標記為已拆分並保留                                   |
| 合併         | 勾選兩筆以上合併成一筆新的待審核原子命題，原原子命題標記為已合併                               |
| 重新抽取本段 | 只針對該段落重新排入抽取工作                                                                   |
| 退回待審核   | 把已核定／已駁回／已合併／已拆分的原子命題退回 pending                                         |

批次功能：批次核定、批次駁回、批次標記待確認，以及「選取可批次核定的 N 筆」
（條件為待審核 + 低風險 + 無品質標記）。

單筆審核畫面同時顯示：可編輯的原子命題欄位、原文片段、**前後文段落**、文件標題與網址、
條件欄位、風險等級、**以三元組相似度找出的相似既有原子命題**，以及完整審核歷程。
...
候選原子命題核定後會立即寫入 `knowledge_facts`，並同時完成三件事：
建立 `fact_versions` 快照、整理主體與客體成 `entities` 與 `relations`、
排入只針對這一筆的向量工作。

```text
核定候選原子命題
  → promote_candidate_fact（拒絕沒有原文片段或未核定的原子命題）
...
`where is_active` 的部分索引）。停用原子命題時其向量一併退出搜尋。
...
- 檢索不到任何核定原子命題時**直接回覆資料不足，完全不呼叫模型**
- 回答若引用證據包以外的知識編號，會記錄為警告（幻覺訊號）
- 證據以快照保存，原子命題日後被修改仍看得到當時用了什麼版本
- 提示詞明文禁止使用模型自身知識、要求保留條件與不確定性、要求標註 `[K-0001]`

Mock Provider 在問答模式下只會引用證據包中的原子命題並附上知識編號，
...
| 判定              | 條件                                                           | 處置                     |
| ----------------- | -------------------------------------------------------------- | ------------------------ |
| 🟢 綠 supported   | 與核定原子命題覆蓋率 ≥ 0.55，數字、否定、條件、語氣全部一致    | 可進入發布稿             |
| 🟡 黃 partial     | 有原子命題支持但語氣被放大、適用條件未保留，或覆蓋率不足       | 保留在發布稿，需人工確認 |
| 🔴 紅 unsupported | 找不到支持原子命題、數字對不上、否定相反、引用不存在的知識編號 | **不得進入發布稿**       |

紅色的具體判定條件：

- 與所有原子命題的覆蓋率都低於 0.25
- 句中數字或單位在支持原子命題中找不到
- 否定與肯定和原子命題相反（「會累積」vs「不會累積」）
- 引用了證據包以外的知識編號（幻覺訊號）

**只要有一句紅色，整份回答就標為 blocked，`published_answer` 不會產生**——
這條規則寫在資料庫函式 `apply_answer_verification` 裡，不是只靠前端隱藏。

條件比對是逐詞比對而非「有沒有條件詞」：原子命題寫「孕婦」、回答只寫「每週」
仍會被標記為條件未保留。

## 風險溝通素材產製

`/generate` 用核定原子命題產製十種素材：FAQ、科普短文、長篇文章、Podcast 訪綱、
Podcast 逐字稿、60 秒短影音腳本、3 分鐘短影音腳本、圖卡文字架構、媒體問答、社群貼文。

```text
主題 → 混合搜尋取出核定原子命題 → 證據包 → 依體裁撰稿
...
素材有體裁結構：小標、題號、秒數、「旁白：」這類文字不是原子命題主張，
...
| 內容                 | 格式                | 說明                                    |
| -------------------- | ------------------- | --------------------------------------- |
| 正式原子命題         | JSON／CSV／Markdown | 只含現行版本，每筆附原文片段與段落      |
| 原子命題與來源對照表 | JSON／CSV／Markdown | 稽核用：原子命題 ↔ 文件 ↔ 段落 ↔ 原文句 |
| 單篇文件與其原子命題 | JSON／Markdown／CSV | 含該版全部段落                          |
| 待選原子命題包       | JSON                | 給其他 LLM 校正（見下節）               |
...
```text
貼上／上傳 JSON → 解析與格式驗證 → 以 id 比對回原本的候選原子命題
...
在對話或其他工具中整理好的一篇文章，可以連同段落、候選原子命題、審核紀錄與正式原子命題
...
正式原子命題一律由候選原子命題經 `promote_candidate_fact` 產生；預設全部以「待審核」匯入，
要沿用檔案中的人工核定結果必須明確勾選。
...
每篇 12 筆候選原子命題（6 核定、2 待修正、2 駁回、2 待審核）與 3 份素材草稿。
