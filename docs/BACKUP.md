...
| 內容                 | 格式                | 用途                             |
| -------------------- | ------------------- | -------------------------------- |
| 正式原子命題         | JSON／CSV／Markdown | 內容備份、交給他人閱讀           |
| 原子命題與來源對照表 | JSON／CSV／Markdown | 稽核：每筆原子命題出自哪一段原文 |
| 單篇文件與其原子命題 | JSON／Markdown／CSV | 單一主題的完整封存               |
| 待選原子命題包       | JSON                | 丟給其他 LLM 交叉校正後回填      |
...
document_chunks       段落
candidate_facts       候選原子命題
review_records        審核歷程
extraction_feedback   抽取問題回報
knowledge_facts       正式原子命題
fact_versions         原子命題版本
entities / relations  實體與關聯
embedding_records     向量（可由原子命題重建，備份可省略）
...
5. 重建向量（不需要還原 `embedding_records`）：
   ...
   `generate-embeddings`，它會為所有沒有現行向量的原子命題補齊。
...
- 正式原子命題：`fact_versions` 與 `knowledge_facts.supersedes` 保留了每一版
- 候選原子命題：`review_records` 記錄了每一次修改前後的內容
...
1. 建一個臨時 Supabase 專案
2. 依 4.1 還原
3. 登入、開 `/knowledge` 確認原子命題數量與正式環境一致
