...
一句話：**所有對外內容都必須能回溯到「經人工核定的事實」與其原始來源片段。**
...
舊版保留、`is_current` 只有一筆。事實永遠指向它成立時的那一版。
...
| 表                    | 用途              | 關鍵欄位                                                                                                                                                                         |
| --------------------- | ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `candidate_facts`     | AI 拆出的候選事實 | `statement`、`subject/predicate/object`、`conditions`、**`source_quote`**、**`source_paragraph_id`**、`risk_level`、`status`、`quality_flags`、`quality_score`、`statement_hash` |
...
| 表                  | 用途                   | 關鍵欄位                                                                                                  |
| ------------------- | ---------------------- | --------------------------------------------------------------------------------------------------------- |
| `knowledge_facts`   | 核定後的正式事實       | 同候選事實 + `tags`、`status`(draft/active/inactive/superseded)、`version`、`supersedes`、`superseded_by` |
| `fact_versions`     | 事實的每一版快照       | `version`、`statement`、`change_note`                                                                     |
| `entities`          | 從主體／客體整理的實體 | `name`、`normalized_name`、`entity_type`、`fact_count`                                                    |
...
`embedding_records.is_active` 是增量更新的關鍵：事實改版時只停用該筆的舊向量、
...
document_chunks ─────────────┐
    ...
    └──────────────► candidate_facts ◄──── prompt_versions
                     ...
                     knowledge_facts ──┬──► fact_versions（每一版快照）
                          ...
                          │  │         └──► relations ──► entities
                          │  │
                          │  └─ supersedes / superseded_by（同一條事實的版本鏈）
                          │
                          ▼ 混合搜尋
...
| 路徑                      | 功能                                                             |
| ------------------------- | ---------------------------------------------------------------- |
| `/dashboard`              | 文件數、候選／核定事實數、待審核、高風險、驗證失敗句數、API 用量 |
| `/sources`                | 匯入（文字／檔案／網址）、清單、解析進度                         |
| `/sources/[id]`           | 版本、段落、重新解析、重新抽取                                   |
| `/import`                 | 原文 + 事實包、事實包單獨匯入、載入示範資料                      |
| `/review`                 | 候選事實清單、篩選、全選待審核、批次核定、回報抽取問題           |
| `/review/[id]`            | 單筆審核：修正、拆分、合併、前後文、相似事實、審核歷程           |
| `/knowledge`              | 正式事實清單、補齊向量                                           |
| `/knowledge/[id]`         | 版本歷程、修改（自動產生新版）、停用                             |
| `/entities`               | 實體清單與事實數                                                 |
| `/relations`              | 實體關聯與支持它的事實                                           |
| `/search`                 | 混合搜尋，可依文件／類型／風險／實體篩選                         |
| `/ask`                    | AI 問答（只用核定事實）、證據包對照                              |
| `/verify`、`/verify/[id]` | 逐句綠黃紅判定、發布稿                                           |
| `/generate`               | 十種素材產製與清單                                               |
| `/generate/[id]`          | 草稿逐句驗證、修改後重新驗證、定稿                               |
| `/export`                 | JSON／CSV／Markdown 匯出、待選事實包與回填                       |
...
| `extract-facts`       | 依 JSON Schema 抽候選事實 → 自動品質檢查 → 寫入        |
| `generate-embeddings` | 只為缺向量的事實產生，寫入前停用該筆舊向量             |
...
| `promote_candidate_fact`    | **只有已核定、且有原文片段的候選事實才能變成正式事實**      |
| `revise_knowledge_fact`     | 修改產生新版本、停用舊向量、只排一筆新向量工作              |
| `search_knowledge_facts`    | 混合搜尋，只回現行事實                                      |
...
| `fact-matching.ts`               | 事實 ↔ 段落比對與引句定位                             |
| `pack.ts`                        | 待選事實包（欄位說明 + 三項校正目標 + 回填格式）      |
...
2. **只有人工核定的候選事實能變成正式事實**
   AI 抽取、外部 LLM 校正回填、文章包匯入——三條路徑的產出一律是待審核。

3. **只要有一句沒有事實支持，整份回答就阻擋**
